...
$('.popup').magnificPopup({
  ...
  --------------------------------------------- */
  var $scholarships = $('.scholarship-gallery');
  if ($.fn.imagesLoaded && $portfolio.length > 0) {
      imagesLoaded($scholarships, function () {
          $scholarships.isotope({
              itemSelector: '.scholarship-item',
              filter: '*'
          });
  ...
  $('.scholarship-filter').on('click', 'a', function (e) {
      e.preventDefault();
      $(this).parent().addClass('active').siblings().removeClass('active');
      var filterValue = $(this).attr('data-filter');
      $scholarships.isotope({filter: filterValue});
  });
